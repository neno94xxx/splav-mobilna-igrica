...
func set_travel(scroll: float, horizontal_scroll: float = 0.0, tile_size: float = 322.0, distance_m: float = 0.0) -> void:
	if is_instance_valid(ocean_material):
		...
		ocean_material.set_shader_parameter("distance_m", maxf(distance_m, 0.0))
